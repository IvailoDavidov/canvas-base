...
export function initCanvas(canvas: HTMLCanvasElement) {
    ...
    function draw() {
        ...
        bkImage.addEventListener(
            "load",
            () => {
                
                ctx.drawImage(bkImage, 300, 450, 50, 50);
               
            },
            false
        );
        bkImage.src = "/assets/bk.png"; // Set source path

        // Create new img element
        wkImage.addEventListener(
            "load",
            () => {
                // execute drawImage statements here
                ctx.drawImage(wkImage, 300, 100,50,50);
            },
            false
        );
        wkImage.src = "/assets/wk.png"; // Set source path
    }
